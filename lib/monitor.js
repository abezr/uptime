 ...
 *   checksInBlock: Max amount of checks performed one by one
 *
...
function Monitor(config) {
  ...
  config.checksInBlock = config.checksInBlock || 50;
  this.config = config;
...
Monitor.prototype.pollChecksNeedingPoll = function (callback) {
    ...
    this.findChecksNeedingPoll(function (err, checks) {
        ...
        var time = new Date();
        exec("", function () {
            self.pollGroup(checks, 0, 0);
        });

        // schedule future polls
        this.intervalForPoll = setTimeout(self.pollChecksNeedingPoll.bind(self), self.config.pollingInterval - (new Date() - time));
    });
};

/**
* Polls groups of checks.
*
* The amount must be appropriate for prevent overloading the network.
*/
Monitor.prototype.pollGroup = function (checks, startCheck, timeOffset) {
  var self = this;
  var checksInBlock = self.config.checksInBlock,
        grousCount = checks.length / checksInBlock,
        subInterval = self.config.pollingInterval / (grousCount + 1);
  checks.slice(startCheck, startCheck + checksInBlock)
        .forEach(function (check) {
          self.pollCheck(check, function (err) {
            if (err) console.error(err);
          });
        });
  if (startCheck < checks.length) {
    setTimeout(function () { self.pollGroup(checks, startCheck + checksInBlock, timeOffset + subInterval) }, timeOffset);
  }
};
